...
export function VariantEDocCompare({ data }: { data: SupersededRecord[] }) {
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (!target.closest('[data-utility-panel]')) {
        setShowFeedback(false); setShowGuidanceHub(false); setShowShortcuts(false)
      }
    }
    document.addEventListener('mousedown', handler)
    return () => document.removeEventListener('mousedown', handler)
    ...
    <div className="flex h-full overflow-hidden bg-background">
      ...
      <div ref={containerRef} className="flex min-w-0 flex-1 flex-col overflow-hidden">
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-border bg-card px-5 py-2.5">
          ...
          {/* Center-right: utility buttons */}
          <div className="flex shrink-0 items-center gap-1">
            {/* Feedback */}
            <div className="relative" data-utility-panel>
              <button
                type="button"
                onClick={() => { setShowFeedback(!showFeedback); setShowGuidanceHub(false); setShowShortcuts(false) }}
                className={`inline-flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-medium transition-colors ${showFeedback ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}
                title="Send feedback"
              >
            ...
            <div className="relative" data-utility-panel>
              <button
                ...
                className={`inline-flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-medium transition-colors ${showGuidanceHub ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}
                title="Guidance hub"
              >
                <BookOpen className="h-3.5 w-3.5" />
                <span className="hidden lg:inline">Guidance Hub</span>
              </button>
              {showGuidanceHub && (
                <div className="absolute right-0 top-full z-50 mt-1.5 w-72 rounded-lg border border-border bg-card p-4 shadow-lg animate-in fade-in-0 slide-in-from-top-1 duration-150">
                  ...
                  <div className="space-y-2">
                    <a
                      ...
                      className="flex items-center gap-3 rounded-md border border-border p-3 text-xs font-medium text-foreground transition-colors hover:bg-muted group"
                    >
            ...
            <div className="relative" data-utility-panel>
              <button
                ...
                className={`inline-flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-medium transition-colors ${showShortcuts ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}
