...
function heading(text, level) {
  level = level || HeadingLevel.HEADING_1
...
function makeTable(headers, rows, widths) {
  return new Table({
    ...
    rows: [
      new TableRow({ children: headers.map(function(h, i) { return headerCell(h, widths[i]) }) }),
    ].concat(rows.map(function(row) {
      return new TableRow({
        children: row.map(function(c, i) { return cell(c, widths[i]) }),
      })
    })),
  })
}
...
async function main() {
  ...
  const outPath = join(__dirname, 'AI-Learning-Feedback-Loop-Spec.docx')
